...
repeat
    ...
    local ret, err = client:brpop(config.redis.pushhug, 0)
    if not ret then
        log.E(common.resp(config.errnu.REDIS_DO_BRPOP_ERROR))
        return
    end
    ...
    --[[
    local off   = ret.off or 0
    local size  = config.max_num.get
    local n     = 0

    while off <= ret.count do
        local t2 = os.time()
        local request_body  = {
            msgId   = ret.msgId,
	    off	    = off,
        }
        copas.addthread(handle, request_body, ret.count)
        if off >= n then
            log.D('push first offset: ' .. off)
            copas.loop()
            log.D('copas loop per 10 threads: ' .. (os.time() - t2))
            n = n + config.max_num.radix
        end
        off = off + size
    end
