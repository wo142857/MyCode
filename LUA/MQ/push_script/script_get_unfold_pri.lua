    ...
    for i = 1, 3 do
        ...
        if code == 200 then
            ...
            if response.Code == 0 and response.Result.rCode == 0 then
                ...
                local r, e = client:lpush(config.redis.getpri,
...
local function handle(msg_tab)
    ...
    repeat
        ...
        if not stop then
            local client = redis_connect()
            local r, e = client:lpush(config.redis.getpri, cjson.encode(msg_tab))
            if not r then
        ...
        if n == 1 then
            -- 首次请求成功后插入下发队列
            local client = redis_connect()
            local r, e = client:lpush(config.redis.pushpri, cjson.encode(msg_tab))
            if not r then
    ...
    if msg_tab.ostype or (msg_tab.ostype == 1) then
        log.D('push to again task, id: ' .. msg_tab.msgId)
        local client = redis_connect()
...
repeat
    ...
    for i = 1, config.max_num.thread_cnt do
        local ret, err = client:rpop(config.redis.getpri)
        if not ret then
            break
        end
